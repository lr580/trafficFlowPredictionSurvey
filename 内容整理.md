...
```tex
\textbf{C}onvolutional \textbf{N}eural \textbf{N}etworks
\textbf{T}emporal \textbf{C}onvolutional \textbf{N}etwork
```
...
>
> ```tex
> \textbf{R}ecurrent \textbf{N}eural \textbf{N}etwork
> ```
...
注意力机制通过模仿人的信号处理策略来实现资 的不平均分配，为不同元素分配不同的权重以选择更有效的信息
...
T-70 Weather data is collected from [JUHE](https://www.juhe.cn/docs/api/id/277) website (国内收费网站)
...
(T-135) 都是 5min 时间步；其论文源码有数据，推荐了 [BasicTS](https://github.com/GestaltCogTeam/BasicTS/tree/master)，其说出处 T-81，边数参见 LibCity 的表格的 #REL 列
...
- [LargeST](https://proceedings.neurips.cc/paper_files/paper/2023/hash/ee57cd73a76bd927ffca3dda1dc3b9d4-Abstract-Datasets_and_Benchmarks.html) [src](https://github.com/liuxu77/LargeST  )
  ...
  We source LargeST from the California Department of Transportation (CalTrans) Performance Measurement System
...
- T-drive [T-97](https://dl.acm.org/doi/abs/10.1145/1869790.1869807) 轨迹数据 3w出租车 北京 2015/2/1 到 6/2
  ...
  T-151 的[开源代码](https://github.com/Pengxin-Guo/ADCSD) 说 [LibCity](https://github.com/LibCity/Bigscity-LibCity) 库有数据及其预处理 [下载](https://www.microsoft.com/en-us/research/publication/t-drive-trajectory-data-sample/)
...
- TaxiBJ T-51 GPS轨迹 3w数据 背景 32x32 网格 [src](https://github.com/Echo-Ji/ST-SSL_Dataset)
  ...
  (T-162) collected in Beijing, spans from 03/01/2015 to 06/30/2015 on an hourly basis, taxi GPS trajectories [src](https://github.com/underdoc-wang/ST-MGCN)
  ...
  [下载2](https://github.com/TolicWang/DeepST/issues/3)
...
- [TaxiNYC](https://www.nyc.gov/html/tlc/html/about/trip_record_data.shtml) 2009 纽约 [src2](https://github.com/Echo-Ji/ST-SSL_Dataset)
  ...
  (T-148 说 [T-149](https://ieeexplore.ieee.org/abstract/document/9127874/) (核查后149为bike)) 75(15x5) 30min 01/01/2014-12/31/2014  据LibCity说数据源是 [T-367](https://dl.acm.org/doi/abs/10.1145/3240508.3240681)
  ...
  NYC-Taxi15 200 2880 01/2015 - 03/2015 30 min (2880是时间步) LibCity 说 T-124
  ...
  T-284 1h 4392时间区间 6800+车 16x8 2014 4-9 bike rent flow (查实与 taxinyc无关，是 bike)
...
- Suzhou SIP taxicab 轨迹 苏州
  ...
  T-362 Suzhou Industrial Park (SIP) (再查发现362没有)
...
- CD chengdu [src](https://goo.gl/3VsEym) speed (其实就是taxiCD)
  ...
  T-307 1.4 billion GPS records from 14,864 taxis collected from 2014-08-03 to 2014-08-30 a total of 3,636,845 trips 
...
- [Digi GAIA Open Data](https://outreach.didichuxing.com/research/opendata/) 滴滴出行
  ...
  - Didi-beijing / shanghai
    ...
    T-306 Nov. 8th 2018 to Dec. 30th 2018 -> T-234 (demand)
...
- [UVDS](https://link.springer.com/chapter/10.1007/978-3-030-73280-6_6) 4 引用 useless 感觉 [T-369](https://link.springer.com/chapter/10.1007/978-3-030-30241-2_46) [T-370](https://link.springer.com/chapter/10.1007/978-3-030-73280-6_6)

  (T-ZS26) 韩国城市地区数据集 UVDS 包含 104 个 VDS 传感 器收集的城市主要道路数据，具有交通流量、车辆类型、交通速度和占用率等交通特征
...
- [韩国 Urban](http://topis.seoul.go.kr/)
  ...
  (T-ZS37) 记录了韩国首尔江南区（Ｕｒｂａｎ１）和麻浦区（Ｕｒｂａｎ２）两个区域的真实车流数据．Ｕｒｂａｎ１和Ｕｒｂａｎ２是首尔交通流量最大的两个地区，且均具有高度复杂的城市交通网络．数据集的采样周期为２０１８年４月１日至２０１８年４月３０日．数据集主要是基于ＧＰＳ采集的７万多辆出租车的轨迹数据，采样时间间隔为５ｍｉｎ．采集的交通流数据经过数据预处理得到的是各链路的平均速度
...
- T-268 成都，西安，深圳，HB speed+flow (但是问题定义说是需求，数据集下载后只看到了速度和需求)
...
[src](https://github.com/guoshnBJTU/ASTGNN)
...
import numpy as np
for x in '3478':
    data = np.load(f'data/PEMS0{x}/PEMS0{x}.npz')
...
```
(26208, 358, 1)
20.00  20.00  182.00  182.00  91.00  
22.00  22.00  174.00  174.00  87.00
22.00  22.00  183.00  183.00  92.00
50.00  49.00  137.00  139.00  60.00
37.00  35.00  128.00  123.00  54.00

(16992, 307, 3)
62.00 0.01 67.90  56.00 0.01 68.40  90.00 0.01 68.20  32.00 0.01 66.30  19.00 0.01 65.40
61.00 0.01 67.30  40.00 0.01 68.60  94.00 0.02 67.80  25.00 0.01 65.70  16.00 0.01 66.80
71.00 0.01 68.40  28.00 0.01 67.40  79.00 0.01 68.70  36.00 0.01 66.60  9.00 0.00 67.20
86.00 0.01 67.80  47.00 0.01 69.50  94.00 0.01 68.90  46.00 0.01 66.80  11.00 0.00 66.30
103.00 0.01 67.40  36.00 0.01 68.10  154.00 0.03 67.30  68.00 0.01 67.00
  9.00 0.00 68.40  

(28224, 883, 1)
246.00  193.00  232.00  260.00  19.00  
249.00  156.00  204.00  236.00  11.00
243.00  158.00  209.00  181.00  10.00
232.00  185.00  218.00  189.00  15.00
243.00  148.00  222.00  195.00  8.00

(17856, 170, 3)
133.00 0.06 65.80  210.00 0.06 69.60  124.00 0.04 65.80  145.00 0.04 69.60  206.00 0.05 69.40
114.00 0.05 66.90  185.00 0.06 68.50  119.00 0.03 65.00  184.00 0.05 69.60  194.00 0.05 68.50
140.00 0.06 66.80  171.00 0.05 69.90  107.00 0.03 63.80  146.00 0.04 73.40  162.00 0.04 68.50
106.00 0.05 68.90  174.00 0.05 68.00  98.00 0.03 63.50  148.00 0.04 71.10  193.00 0.05 70.20
117.00 0.06 67.40  176.00 0.05 68.70  114.00 0.03 63.10  130.00 0.04 68.90  163.00 0.05 66.10
```

发现第三维形状不一样，对 4/8 不一样，且第三维第一个数字一定都是整数。

> 对 PEMSD4，根据 [视频(ASTGCN)](https://www.bilibili.com/video/BV16R4y197Fs/?spm_id_from=333.337.search-card.all.click&vd_source=3f13edc74e77991f90a1b89b0e936066) 在 3 分钟的代码注释，三个数据特征分别是：流量、速度、占用率。意思来源就是 ASTGCN。而另外两个，根据 LibCity，来源疑似为 STSGCN。

###### 图结构探讨

CSV 的 PEMS03 有空行，且暂未解读明白 CSV 的 cost/distance 的单位/含义。有待研究。也不明白是有向图还是无向图。

连通性：PEMSD7 有 17 个连通块

```python
path = 'D:\Temps\chormedf\ASTGNN-main\ASTGNN-main\data\PEMS07'
import pandas as pd
import os
edges = pd.read_csv(os.path.join(path, 'PEMS07.csv'))
g = [[] for i in range(edges['from'].max()+1)]
for i in range(edges.shape[0]):
    u = edges.iloc[i, 0]
    v = edges.iloc[i, 1]
    w = edges.iloc[i, 2]
    g[u].append((v, w))
    g[v].append((u, w)) # dfs 可知，有向图不连通
# V0 = edges.iloc[:100, 0]
# g0 = {k:[] for k in V0}
# for u in V0:
#     for v, w in g[u]:
...
```python
import numpy as np
# data = np.load(r'D:\Temps\chormedf\adj_mx.npz')
# print(data.files)
# print(data['adj_mx'].shape)
# print(data['adj_mx'].dtype)
# print(data['adj_mx'])
data = np.load(r'D:\Temps\chormedf\val.npz')
print(data.files)
...
对 a 的输出发现不是 0 就是 1，一开始以为是数据问题，~~遇到困难睡大觉，游泳回来后~~发现可能是深夜数据，所以拉长来看整个 val 看看。后来发下是可能选了最边边的区域，换成 100 效果显著。
...
for t in range(x.shape[0]):
    for v in range(x.shape[1]):
        for i in range(x.shape[-1]):
            res += str(x[t, v, i])
        res += ' ' 
    res += " - " + str(y[t, 0, 0]) + ' ' + str(y[t, 0, 1]) + '\n'
with open('res.txt', 'w') as f:
    f.write(res)
```

~~我真是笨死了，~~后来发现，原来在 [here](https://github.com/Echo-Ji/ST-SSL_Dataset) 的 README.md 描述了数据含义。解读如下：
...
- [BasicTS](https://github.com/GestaltCogTeam/BasicTS) 时序汇总，含数据等
- [LibCity](https://github.com/LibCity/Bigscity-LibCity) 交通预测库，含数据、论文等
